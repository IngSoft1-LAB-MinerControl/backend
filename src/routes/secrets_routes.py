...
async def reveal_secret(game_id : int, secret_id : int , player_id : int , db: Session = Depends(get_db)):
...
async def hide_secret(game_id : int, secret_id : int , player_id : int , db: Session = Depends(get_db)):
    # 2. Llamar a la función de servicio con los parámetros recibidos
    # La función de servicio se encarga de toda la lógica y las excepciones.
...
async def steal_secret(game_id : int, secret_id : int , player_id : int , target_player_id: int, db: Session = Depends(get_db)):
    # 2. Llamar a la función de servicio con los parámetros recibidos
    # La función de servicio se encarga de toda la lógica y las excepciones.
