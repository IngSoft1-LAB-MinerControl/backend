...
def play_set_of2(card_id : int , card_id_2:int , db:Session=Depends(get_db)):
    ...
    if not card_1 or not card_2:
        return HTTPException(status_code=400, detail=f"Invalid card_id")
  
    if (card_1.quantity_set > 2 or card_2.quantity_set > 2):
        return HTTPException(status_code=400, detail=f"You need one more detective to play this set")
    ...
    if (card_1.name == "Harley Quin Wildcard"):
        ...
        new_set = Set(name = "Bereford brothers" , 
                      player_id = card_1.player_id ,
                      game_id = card_1.game_id)
        db.add(new_set)
        try:
        ...
        return HTTPException(status_code=400, detail=f"This are not two compatible detectives")
